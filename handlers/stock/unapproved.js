...
module.exports = function (req, res, next) {
	...
	return req.mssql.request()
			...
			.query("SELECT " + (limit && limit > 0 ? "TOP " + limit + " " : "") + "dbo.Stock.RECID, dbo.Stock.ITEMNO, dbo.Stock.PGROUP, dbo.Stock.GRPCODE, dbo.Stock.LASTSER#3, dbo.Stock.PERIOD#1, dbo.Stock.CURRDEPOT, dbo.Stock.DESC#1, dbo.Stock.DESC#2, dbo.Stock.DESC#3 FROM dbo.Stock WHERE dbo.Stock.LASTSER#3 LIKE '%1899%' AND dbo.Stock.PATTEST = '1'" + (status.length ? " AND dbo.Stock.STATUS IN(@status)" : "AND dbo.Stock.STATUS = 100") + " ORDER BY DESC#1 ASC")
      .then(result => {
